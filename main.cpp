...
void rectangleLUij(TYPE* A, const int m, const int n, const int i0, const int j0) //алгоритм 2.9 Деммель
{
	int min = MIN(m - 1, n);
	for (int i = 0; i < min; ++i)
	{
...
void rectangleLUijParallel(TYPE* A, const int m, const int n, const int i0, const int j0) //алгоритм 2.9 Деммель
{
	int min = MIN(m - 1, n);
	for (int i = 0; i < min; ++i)
	{
...
#pragma omp for 
			for (int j = 0; j < N - b - i; ++j)
			{
				...
				for (int k = 0; k < b; ++k)//Обратный ход метода Гаусса
				{
					TYPE sum = 0;
					for (int q = 0; q < k; ++q) {
...
int main()
{
	...
	cout << "\n\t\tСтандартное LU-разложение параллельно" << endl;
	tm = LUDecompositionParallel(LU);	// A -> LU
	cout << "Время: " << tm << endl;
	if (calculateErrors) {
		er = getError(LU, Ad);
		cout << "Ошибка ||LU-A||: " << er << endl;
		memcpy(Ad, A, N * N * sizeof(TYPE));
	}
	memcpy(LU, A, N * N * sizeof(TYPE));
	...
	cout << "\n\t\tБлочное LU-разложение параллельно" << endl;
	tm = LUDecompositionBlockParallel(LU);	// A -> LU
	cout << "Время: " << tm << endl;
	if (calculateErrors) {
		er = getError(LU, Ad);
		cout << "Ошибка ||LU-A||: " << er << endl;
